...
class Day06 {
    ...
    private record Race(int t, int d) {
        private int solveCount() {
            ...
            var start = (int)Math.ceil(Math.max(Math.min(epsilon + s0, t), 0.0));
            var end = (int)Math.floor(Math.max(Math.min(-epsilon + s1, t), 0.0));
            return end + 1 - start;
        }
    }
    ...
    private static Race[] parse() {
        ...
        var times = Arrays.stream(lines.get(0).split(":")[1].trim().split(" ")).filter(s -> s.length() != 0).map(Integer::parseInt).toList();
        var distances = Arrays.stream(lines.get(1).split(":")[1].trim().split(" ")).filter(s -> s.length() != 0).map(Integer::parseInt).toList();
